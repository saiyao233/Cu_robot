...
parser.add_argument("--robot", type=str, default="dual_franka.yml", help="robot configuration to load")
...
def main():
    ...
    robot, robot_prim_path = add_robot_to_scene(robot_cfg, my_world,position=[0, 0, 0])
    ...
    ee_link_name=mpc.kinematics.ee_link
    kin_state = mpc.kinematics.get_state(mpc.get_retract_config().view(1, -1))
    ...
    while simulation_app.is_running():
        ...
        if (np.linalg.norm(cube_position - past_pose) > 1e-3
            or np.linalg.norm(c_p - past_pose2) > 1e-3
            ):
